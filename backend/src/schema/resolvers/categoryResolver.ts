...
export const categoryResolvers = {
  Query: {
    async getAllCategories(_: unknown, __: never, context: MyContext) {
      try {
        if (!context.user) {
    ...
    async getCategoryById(_: unknown, { id }: { id: string }, context: MyContext) {
      try {
        if (!context.user) {
          logger.error(
            "Error while fetching category by ID: Authentication required",
            context.user
          );
          throw new GraphQLError("Authentication required", {
            extensions: { code: "UNAUTHENTICATED", status: 401 },
        ...
        logger.info(
          `Fetched category with ID ${id} successfully name ${category.name} by `,
          context.user
        );
  ...
  Mutation: {
    async addCategory(_: unknown, { input }: { input: AddCategoryInput }, context: MyContext) {
      try {
        //  Check if user is authenticated and is an Admin
        if (!context.user || context.user.role !== "Admin") {
          logger.warn(
            `[addCategory] Unauthorized access attempt by user ${context.user?.id || "Unknown"}`
          );
    ...
    async deleteCategory(
      ...
      try {
        ...
        if (context.user.role !== "Admin") {
          logger.warn(
            `[deleteCategory] User ${context.user.id} attempted to delete a category without admin privileges.`
          );
        ...
        // Prevent deletion if linked to menu items
        const linkedMenuItems = await MenuItem.findOne({ categoryId: id });
        if (linkedMenuItems) {
          logger.warn(
            `[deleteCategory] Cannot delete category (${id}) - ${linkedMenuItems} menu items linked.`
          );
          throw new GraphQLError(
            "Cannot delete category with existing menu items. Remove menu items first.",
            {
              extensions: { code: "BAD_REQUEST", status: 400 },
            }
          );
    ...
    async updateCategory(
      ...
      try {
        //  Ensure the user is authorized to update categories (Admin role)
        const { id, name } = input;
        ...
        if (context.user.role !== "Admin") {
          logger.warn(
            `Unauthorized access attempt by user ${context.user?.id} to update category ${id}`
          );
        ...
        if (!name || typeof name !== "string" || name.trim() === "") {
          logger.error(
            `Invalid input: Category name must be a non-empty string. Input: ${JSON.stringify(
              input
            )}`
          );
        ...
        //  Check if there's a conflicting category name
        ...
        throw new GraphQLError(
          error.message || "An unexpected error occurred while updating the category.",
          {
