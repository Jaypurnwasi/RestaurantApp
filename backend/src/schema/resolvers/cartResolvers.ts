...
export const cartResolvers = {
  Query: {
    async getAllCartItems(parent: unknown, __: unknown, context: MyContext) {
      try {
        ...
        const cart: PopulatedCart | null = await Cart.findOne({
  ...
  Mutation: {
    async addItemToCart(_: unknown, { input }: { input: AddToCartInput }, context: MyContext) {
      try {
        // Ensure the user is authenticated
        if (!context.user) {
          throw new GraphQLError("Authentication required", {
        ...
        return {
          menuItemId,
          quantity:
            cart.items.find((item) => item.menuItemId.equals(menuItemId))?.quantity || quantity,
        };
      } catch (error: any) {
    ...
    async decreaseItemQuantity(
      _: unknown,
      { input }: { input: DecreaseQuantityInput },
      context: MyContext
    ) {
      try {
        // Authentication Check
    ...
    async clearCart(_: unknown, __: any, context: MyContext) {
      try {
        // Authentication Check
        if (!context.user) {
          throw new GraphQLError("Authentication required", {
