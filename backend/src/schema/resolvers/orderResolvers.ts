...
import { MyContext, CreateOrderInput, UpdateOrderStatusInput } from "../types/types";
...
export const orderResolvers = {
  Query: {
    getOrderById: async (_: unknown, { orderId }: { orderId: string }, context: MyContext) => {
      try {
        if (!context.user) {
        ...
        if (
          ...
          order.customerId._id.toString() !== context.user.id
        ) {
    ...
    getAllOrders: async (_: unknown, __: never, context: MyContext) => {
      try {
        if (!context.user) {
          throw new GraphQLError("Authentication error: User not authenticated", {
            extensions: { code: "UNAUTHORIZED" },
        ...
        return orders.map((order) => ({
          ...
          customerName:
            order.customerId && typeof order.customerId !== "string"
  ...
  Mutation: {
    createOrder: async (_: unknown, { input }: { input: CreateOrderInput }, context: MyContext) => {
      try {
        // Ensure user is authenticated
        if (!context.user) {
          throw new GraphQLError("Authentication error user not authenticated", {
            extensions: { code: "token not found" },
          });
        }
        ...
        for (const item of cart.items) {
          ...
          if (!menuItem) {
            throw new GraphQLError(`Menu item not found: ${item.menuItemId}`, {
              extensions: { code: "BAD_REQUEST" },
            });
          }
        ...
        if (!existingTable) {
          throw new GraphQLError("table does not exists", {
            extensions: { code: "BAD_REQUEST" },
          });
        }
